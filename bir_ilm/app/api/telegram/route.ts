...
export async function POST(req: NextRequest) {
  try {
    ...
    if (text.startsWith("/makeadmin ")) {
      if (!isSuperAdmin) {
        await sendTelegram(chatId, "Bu buyruq faqat super admin uchun.");
        return NextResponse.json({ ok: true });
      }
      const targetId = text.replace("/makeadmin ", "").trim();
      await fetch(`${SUPABASE_URL}/rest/v1/users?telegram_id=eq.${targetId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json", "apikey": SUPABASE_KEY, "Authorization": `Bearer ${SUPABASE_KEY}` },
        body: JSON.stringify({ role: "admin" }),
      });
      await sendTelegram(chatId, `Admin berildi: ${targetId}`);
      return NextResponse.json({ ok: true });
    }
    ...
    if (text.startsWith("/removeadmin ")) {
      ...
      await sendTelegram(chatId, `Admin olib tashlandi: ${targetId}`);
      return NextResponse.json({ ok: true });
    }

    if (text === "/adminlar") {
      const res = await fetch(`${SUPABASE_URL}/rest/v1/users?role=in.(admin,super_admin)&select=username,telegram_id,role`, {
        headers: { "apikey": SUPABASE_KEY, "Authorization": `Bearer ${SUPABASE_KEY}` },
      });
